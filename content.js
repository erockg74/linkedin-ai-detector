...
(function () {
  ...
  // ─── Composer "Score with AI" button ────────────────────────────────
  // LinkedIn's post composer lives inside #interop-outlet's shadow DOM.
  // We watch for the modal to appear, then inject our button next to
  // LinkedIn's "Rewrite with AI" in the toolbar carousel.

  const COMPOSER_BTN_ID = "ai-detector-score-draft-btn";

  function getComposerShadow() {
  ...
  function injectComposerButton() {
    ...
    // Already injected?
    if (shadow.getElementById(COMPOSER_BTN_ID)) return;

    // Find "Rewrite with AI" button as anchor (Premium), or fall back to
    // the first carousel item (non-Premium users still have the toolbar).
    const allBtns = shadow.querySelectorAll("button");
    let rewriteBtn = null;
    for (const b of allBtns) {
      if (b.textContent.includes("Rewrite with AI")) { rewriteBtn = b; break; }
    }

    let anchorLi = rewriteBtn?.closest("li.artdeco-carousel__item") || null;
    if (!anchorLi) {
      // Non-Premium fallback: find the carousel and use its first LI
      const carousel = shadow.querySelector("ul.artdeco-carousel__slider");
      anchorLi = carousel?.querySelector("li.artdeco-carousel__item");
    }
    if (!anchorLi) return;
    ...
    btn.type = "button";
    btn.className = "artdeco-button artdeco-button--muted artdeco-button--1 artdeco-button--secondary";
  ...
  function showComposerResult(shadow, anchorBtn, score, reason) {
    ...
    const existing = shadow.getElementById("ai-detector-composer-result");
    if (existing) existing.remove();

    const popup = document.createElement("div");
    popup.id = "ai-detector-composer-result";
    popup.style.cssText =
      "position: absolute; z-index: 10000; padding: 10px 14px; border-radius: 10px; " +
  ...
  // on the shadow root's host element.
  function watchComposer() {
    const outlet = document.getElementById("interop-outlet");
    if (!outlet) {
      // Retry — outlet might not exist yet on initial page load
      setTimeout(watchComposer, 2000);
      return;
    }
    ...
    // Observe the shadow DOM for composer modal appearing
    const composerObserver = new MutationObserver(() => {
      injectComposerButton();
    });
    composerObserver.observe(shadow, { childList: true, subtree: true });
  }

  // Start watching after a short delay to let LinkedIn render
